...
#Transform the data
flirt -in "$bedpostfolder/${n}/FDTnew/pop/fdt_paths.nii.gz" -applyxfm -init "$outputfolder/$n/dti_fin.mat" -out "$outputfolder/$n/new_dti_mni_pop" -paddingsize 0.0 -interp trilinear -ref "/scratch/braindata/DSmirnov/HarvardOxford/MNI152_T1_2mm_brain.nii"

flirt -in "$bedpostfolder/${n}/FDTnew/ptr/fdt_paths.nii.gz" -applyxfm -init "$outputfolder/$n/dti_fin.mat" -out "$outputfolder/$n/new_dti_mni_ptr" -paddingsize 0.0 -interp trilinear -ref "/scratch/braindata/DSmirnov/HarvardOxford/MNI152_T1_2mm_brain.nii"

flirt -in "$bedpostfolder/${n}/FDTnew/ag/fdt_paths.nii.gz" -applyxfm -init "$outputfolder/$n/dti_fin.mat" -out "$outputfolder/$n/new_dti_mni_ag" -paddingsize 0.0 -interp trilinear -ref "/scratch/braindata/DSmirnov/HarvardOxford/MNI152_T1_2mm_brain.nii"
